// Node03 (client) need transmit to server for send data to master
#include <RF24.h>
...
int fail_forward_feedback = 0;        // variabel untuk menyimpan waktu timer gagal pengiriman/feedback delay
...
bool stat_failed_data = false;        // untuk mengaktifkan/nonaktifkan pengiriman data gagal
...
void loop()
{
    RF24NetworkHeader headerToServer_1(node_server_01);
    ...
    if (stat_just_one_time_send)
    {
        ...
        bool ok = network.write(headerToServer_1, &Data, sizeof(Data));
        // Jika data berhasil dikirim, maka
        if (ok == true)
        {
            Serial.println("------FORWARD BY SERVER 1------");
        ...
        {
            Serial.println("------FORWARD TO NODE SERVER 2------");
            // kirim array Data ke node_server_02 dan assign returnnya ke variabel boolean ok
            bool ok = network.write(headerToServer_2, &Data, sizeof(Data));
            // Jika data berhasil dikirim, maka
            if (ok == true)
            {
                Serial.println("------FORWARD BY SERVER 2------");
                if (!radio.isAckPayloadAvailable())
                {
                    Serial.print("From\t: ");
            ...
            else
            {
                ...
                Serial.print("Weight\t: ");
                Serial.println(Data[weight]);
                // hitung iterasi berapa kali percobaan pengiriman yang gagal
                failed_send_to_branch_stack++;
                // aktifkan kondisi stat failed data untuk menjalankan timer 2 detik untuk mencoba ulang pengiriman data
                Serial.print("Try\t: ");
                Serial.println(failed_send_to_branch_stack);
